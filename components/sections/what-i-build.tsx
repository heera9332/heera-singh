    ...
    <section className="py-20 bg-background/50 border-b border-border/50">
      <Container>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {BUILD_CATEGORIES.map((cat, index) => {
            const Icon = cat.icon;
            return (
              <AnimateIn
                key={cat.title}
                ...
                className="h-full"
              >
                <div className="group relative flex flex-col justify-between h-full rounded-2xl border border-border/80 bg-card/60 p-6 transition-all duration-200 hover:border-primary/50 hover:bg-card hover:shadow-lg hover:shadow-primary/5">
